...
class Vendor:
    def __init__(self, code, name, average_calories, area, location, location_code, description, rating,
                 image_location):
        self.__code = code
        self.__name = name
    ...
    def get_info(self):
        ...
        if self.food_items != []:
            ...
            for food in self.food_items:
                food_date = datetime.strptime(food['datetime(created, "localtime")'], '%Y-%m-%d %H:%M:%S').strftime(
                    '%d-%m-%y')
            ...
            for date in self.__all_dates:
                ...
                for food in self.food_items:
                    if date == datetime.strptime(food['datetime(created, "localtime")'], '%Y-%m-%d %H:%M:%S').strftime(
                            '%d-%m-%y'):
        ...
        for food in self.food_items:
            if 5 <= int(
                    datetime.strptime(food['datetime(created, "localtime")'], '%Y-%m-%d %H:%M:%S').strftime('%H')) <= 9:
            ...
            elif 11 <= int(datetime.strptime(food['datetime(created, "localtime")'], '%Y-%m-%d %H:%M:%S').strftime(
                    '%H')) <= 14:
                self.__lunch_list.append(food['calories'])
                self.__average_lunch_calories = round(sum(self.__lunch_list) / self.__number_of_days, 2)
            elif 17 <= int(datetime.strptime(food['datetime(created, "localtime")'], '%Y-%m-%d %H:%M:%S').strftime(
                    '%H')) <= 21:
